...
#define DEFINE_SPIRV_ImageQuerySizeLod_2d(ImgTy)                               \
  __attribute__((overloadable, always_inline)) int2 __spirv_ImageQuerySizeLod( \
      ImgTy img, int lod) {                                                    \
    return get_image_dim(img) >> lod;                                          \
  }

#define DEFINE_SPIRV_ImageQuerySizeLod_2darray(ImgTy)                          \
  __attribute__((overloadable, always_inline)) int3 __spirv_ImageQuerySizeLod( \
      ImgTy img, int lod) {                                                    \
    return (int3)(get_image_dim(img) >> lod,                                   \
                  get_image_array_size(img) >> lod);                           \
  }

#define DEFINE_SPIRV_ImageQuerySize_2d(ImgTy)                                  \
  __attribute__((overloadable, always_inline)) int2 __spirv_ImageQuerySize(    \
      ImgTy img) {                                                             \
    return get_image_dim(img);                                                 \
  }

#define DEFINE_SPIRV_ImageQuerySize_2darray(ImgTy)                             \
  __attribute__((overloadable, always_inline)) int3 __spirv_ImageQuerySize(    \
      ImgTy img) {                                                             \
